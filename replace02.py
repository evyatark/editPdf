...
def add_text(page, blocks, font_buffers, new_fontnames):
    ...
    write_all_text(page, textwriters)


def write_all_text(page, textwriters):
...
def add_text_to_line(page, textwriters, line, font_buffers, new_fontnames):
    ...
    for span in line["spans"]:
        ...
        if new_fontname is None:  # do not replace this font
            continue
        # new_fontname now causes same font as the original font!
        # we do not replace fonts anymore,
        # but we use the font-replacement mechanism to add font files from thw Windows OS
        # (for example C:/Windows/Fonts/arial.ttf if ArialMT font is used in PDF)
        # of course this assumes running on a Windows OS (later will be fixed for Linux)
